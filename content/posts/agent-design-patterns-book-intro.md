...
大模型有一个绕不开的特性：**它是概率性的**。同样的输入，可能给出不同的输出；它会一本正经地胡说，也会在关键时刻掉链子。可我们偏偏想用它来搭建**可靠**的系统——能被信任、能上生产、能对结果负责的系统。
...
黄佳的《Agent 设计模式：图解可复用智能体架构》（人民邮电出版社）给出的答案，不是某个技巧，而是一整套方法论。下面我会沿着这条主线，把全书拆成四个环环相扣的问题：

1. **为什么是 Agent？**（上篇）——软件工程三十年，如何一步步逼近"用不确定性构建可靠系统"这个答案
2. **Agent 靠什么运转？**——目标、上下文、记忆、反思……五条设计原则撑起整个范式
3. **可靠性从哪来？**（下篇）——感知、记忆、推理、行动、反思、协作，六大类能力如何各自消解一部分不确定性
4. **落到实处是什么？**——21 个可复用的设计模式，把上面的答案变成能抄的工程范式
...
在展开历史与模式之前，先立住一个"元框架"。如果说核心命题是"在不确定上求可靠"，那么这五条原则就是通往答案的**五个方向盘**——它们贯穿全书，也贯穿下文每一个设计模式：
...
**→ 消解「逻辑跳步」的不确定性。** 记忆解决"知道什么"，推理解决"如何运用"。
...
绕了一圈，我们回到开篇那个问题：**如何在不确定的大模型上，构建可靠的系统？**

这本书给出的答案，其实很朴素——**你不可能消灭不确定性，但你可以驾驭它。** 不是靠更强的控制，而是靠一套让系统能感知、能记忆、能推理、能行动、能反思、能协作的架构。可靠性不再来自"每一步都对"，而来自"错了能发现、发现能纠正、纠正能沉淀"。
