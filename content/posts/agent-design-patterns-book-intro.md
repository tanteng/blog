...
- **微服务 API**：为 Agent 调用工具提供了网络基础设施——每个工具都是可寻址的网络端点
...
| **类比** | 精确控制每一块砖的位置 | 设定土壤成分，等待植物自然生长 |

**关键洞察**：K8s 的 Controller 已经在实践 Agent 的核心思想——**声明期望状态，持续调谐直到达成**。LLM Agent 不过是把这个模式搬到了业务逻辑层。
...
**MLOps 的启示**：当代码演变为数据（模型权重），传统 DevOps 工具链力不从心——你需要监控的不再是服务器指标，而是模型性能分布。**MLOps（Machine Learning Operations）** 是一套管理机器学习模型全生命周期的实践和方法，涵盖数据准备、模型训练、部署、监控和迭代，类似于 DevOps 面对传统软件的作用，MLOps 解决了 AI 系统从实验到生产的最后一公里问题，也为 AgentOps 铺平了道路。
