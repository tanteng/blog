...
这条 Policy 可能是当初在腾讯云控制台配置 CDN 回源时，不小心选了"公有读写"预设模板后自动生成的；也可能是使用 AI Coding 工具（如 OpenClaw）进行网站优化时，AI 自行编写并写入的——**其来源已经无法完全追溯，但它的存在把桶的写入口彻底开放给了所有人。**
...
Policy 的来源已经无法完全追溯，可能是控制台操作时误选了预设模板，也可能是 AI Coding 工具进行网站优化时 AI 自行写入的。但无论来源如何，它的实际效果是把本该"公有读私有写"的桶变成了"公有读写"，让灰产有了可乘之机。
