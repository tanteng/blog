...
另一个深刻感受是 **AI 编程工具对这类迁移工作的加速效果非常明显**。这次迁移涉及的技术面很广——存储 API 适配、图片批处理、部署自动化、CDN 缓存策略——每个环节都需要不同领域的知识。以前可能要查好几轮文档才能搞定的事，现在在 WorkBuddy 里描述清楚需求，Opus 4.6 生成初版代码，自己 review 一遍微调参数就能用。模型能力的差异在实际工程中体现得很明显：同样的任务，Claude Opus 4.6 对复杂代码库的理解深度和生成质量，比我在 OpenClaw 中用的 MiniMax-2.5 更强一些。选对工具和模型，真的能让效率翻倍。
