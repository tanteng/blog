---
title: "告别 Vercel：将 Next.js 照片博客迁移到腾讯云自托管"
date: 2026-03-22T08:00:00+08:00
draft: false
tags: ['vercel', 'tencent-cloud', 'next.js', 'migration', 'lighthouse']
categories: ['tech']
description: "记录将基于 exif-photo-blog 的 Next.js 照片站点从 Vercel 全家桶迁移到腾讯云轻量应用服务器自托管的完整过程，涵盖计算、存储、数据库、CDN 各层面改造"
---

本文记录了将基于 [exif-photo-blog](https://github.com/sambecker/exif-photo-blog) 的照片站点从 Vercel 全家桶迁移到腾讯云轻量应用服务器自托管的完整过程，涵盖计算、存储、数据库、CDN 等各个层面的改造。
...
最终决定整体迁移到腾讯云轻量应用服务器，把计算、存储、数据库全部拉到广州区域的一台轻量服务器上。
...
| 本地 PostgreSQL | 0（复用现有服务器）| < 1ms | 同一台轻量服务器，无需额外费用 |

考虑到照片数量只有 400+ 张，数据量很小，单独购买云数据库性价比不高。直接复用现有轻量应用服务器本地部署 PostgreSQL，既省钱延迟又最低。

### 2.2 迁移动机

之前用的是 Neon PostgreSQL，部署在 AWS us-east-1。从广州的服务器到美东的数据库，每次查询光网络往返就 200ms+，构建时几百次查询累积下来非常痛苦。

### 2.3 安装与数据导入

在服务器上安装 PostgreSQL 15.16，然后从 Neon 导出导入：
...
2. **rclone 内网传输**：腾讯云轻量服务器到 COS 用内网 endpoint，免费且快
