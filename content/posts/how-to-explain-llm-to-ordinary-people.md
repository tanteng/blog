...
**关键：答案是哪里来的？答案是文字本身**。随便找一句话「今天天气真好」，把「好」藏起来让模型猜——「好」就是这一轮的答案，不需要任何「老师」来标。这就是为什么大模型能用几万亿字的语料训练——**「答案」免费**。

你可能还会问：「只能猜下一个字吗？能不能遮住前面的、中间的？」**都可以**。现代训练其实有更花哨的玩法——遮住中间的字让模型猜（更擅长理解上下文）、遮住整段话让模型补全（更擅长翻译摘要）。但**核心思想都一样**：让模型在「看到一部分、不知道一部分」的情况下，猜出被藏的内容。练到最后，它学会的是同一件事——「语言的统计规律」。

整个训练分两步：

- **第一步：让它会说话**——给它读海量文字，自学「什么词后面接什么」（文字本身就是答案）
