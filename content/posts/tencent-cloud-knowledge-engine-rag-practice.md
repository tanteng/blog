...
- **同步解析有耗时限制**，SSE 接口不是为超大文件设计的，PDF 上百页 + 复杂表格偶尔会超时。批量入库老老实实用异步
- **语义拆分有时候过度切分**，文档里有个表格被切到两个 chunk 里，召回的时候丢一半上下文。解决办法是拆分后做一次"小 chunk 合并"后处理
- **Embedding 一次性 batch 上限**，几万个 chunk 一次性调用会被截断。自己分批 + 加并发控制
- **Rerank 加了之后 P95 延迟上去了**，要监控。如果延迟是硬指标，可以考虑异步预 rerank 或者减小候选集大小
- **多轮改写在单轮场景引入噪音**，前面说过，按需开启
- **知识库全量重建很贵**，按文档做增量更新是企业场景的硬需求

这些坑大多数不是"原子能力"本身的限制，而是任何 RAG 系统都会遇到的工程问题。用腾讯这套 API 至少把上游（解析、拆分）的质量托住了，可以把精力放在下游的检索效果和业务编排上。
