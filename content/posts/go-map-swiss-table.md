...
Swiss Table 使用的不是拉链法，而是**开放寻址**。其核心创新是将桶分为多个 **Group**，每个 Group 有 **16 个 Slot**。整个结构分为两部分：

- **metadata array（控制字节数组）**：存储每个 Slot 的控制信息
- **slots array（数据槽数组）**：存储实际的 key-value 数据
...
- **H1**：哈希值的高位 bit，用于确定该元素属于哪个 Group
- **H2**：哈希值的低 7 位，作为**哈希指纹**存储在 metadata 中，用于快速比对

以 64 位哈希值为例：最高 57 位为 H1，最低 7 位为 H2。H2 会被存入对应 Slot 的控制字节中，由于控制字节只有 7 位可用空间，正好存放 H2。

### 控制字节的三种状态

每个 Group 的 metadata 是一个 16 字节数组，每个字节对应一个 Slot 的控制字节：

| 控制字节 | 状态 |
|----------|------|
| `1xxxxxxx`（最高位为1，其余全零） | **空闲（Empty）**，该 Slot 从未被占用 |
| `0xxxxxxx`（最高位为0，后7位为H2指纹） | **已使用（Full）**，该 Slot 存有数据 |
| `11111110` | **已删除（Tombstone）**，该 Slot 曾有数据但已删除，可重新使用 |

### 查找过程

Swiss Table 的查找过程分为两步：

1. **定位 Group**：通过 H1 计算出目标 Slot 所在的 Group 起始位置
2. **Group 内探测**：在 Group 内通过 SIMD 指令**并行比较** 16 个控制字节中的 H2 指纹，快速找到匹配项

具体来说，SIMD 指令可以一次性读取 128 位（16 字节），与 Group 的 16 个控制字节进行匹配，快速排除不可能的 Slot。这就像「布隆过滤器」一样，能快速排除不匹配的项，减少不必要的内存访问。

如果当前 Group 没有找到匹配项，则继续探测下一个 Group。

### 插入过程

1. 计算 key 的哈希值，拆分出 H1 和 H2
2. 定位到对应的 Group
3. 在 Group 内找到第一个**空闲（Empty）**的 Slot，将数据写入对应的 slots array
4. 将 H2 指纹写入对应 Slot 的控制字节

如果当前 Group 已满，则顺延到下一个 Group 继续寻找空位。

### SIMD 优化的原理

16 个条目的分组大小并非随意选择，而是基于：

- **SSE2 寄存器长度**：128 bit = 16 bytes，可一次性操作 16 个控制字节
- **CPU 缓存行大小**：64 字节，保证一个 Group 的控制字节能被单次 SIMD 指令处理

这使得 Swiss Table 在保持高内存利用率的同时，实现了接近 O(1) 的查找性能。
