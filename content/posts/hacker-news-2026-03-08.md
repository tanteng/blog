...
CasNum 是一个有趣的开源项目（GitHub 270 points），是一个与数字/计算相关的工具。这是一个 Show HN 项目，表明开发者社区对此有较高兴趣。
...
这是 2026 年度云虚拟机性能与价格对比评测，涵盖了 7 家主流云服务商（包括 AWS、GCP、Azure、Oracle、Akamai Linode、DigitalOcean、Hetzner）的 44 种 VM 类型。评测涵盖单线程和多线程性能，并计算了按需付费、1 年/3 年预留实例和 Spot 实例的性价比。

新增了 AMD EPYC Turin 和 Intel Granite Rapids 等新 CPU，以及 Google Axion、Azure Cobalt 100 和 Ampere AmpereOne M 等 ARM 解决方案。评测显示 AMD Turin 在高端性能上表现突出。
...
Python 包管理工具 uv 的维护者在 PR 中提出警告：PyPy 可能已经不再被积极开发维护。根据 PyPy 开发者在 numpy 项目中的讨论，PyPy 正在被 numpy 等主流库逐步淘汰。信息，帮助用户避免uv 通过添加警告误以为 PyPy 是正常维护的 Python 发行版。
...
这是一篇关于文件系统在 AI 时代重要性的思考文章。随着 AI 代理（agents）的发展，文件系统和文件交互成为人类与 AI 协作的核心界面。文章探讨了为什么文件系统设计对 AI 应用至关重要，以及如何构建更好的文件交互体验来支持 AI 工作流。
...
Unsloth.ai 发布了本地运行阿里 Qwen3.5 模型的完整指南。Qwen3.5 是阿里云推出的新一代大语言模型系列，包括 35B-A3B、27B、122B-A10B、397B-A17B 以及小型版本（0.8B、2B、4B、9B）。该系列支持 256K 上下文长度、201 种语言，具备思考模式和非思考模式切换功能。

指南详细列出了不同量化版本（3-bit 到 BF16）的硬件需求，例如 27B 模型最低需要 14GB 内存（3-bit）到 54GB（BF16），35B 需要 22GB（4-bit），可以在 22GB Mac 上运行。

链接: https://news.ycombinator.com/item?id=47292522
