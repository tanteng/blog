...
| | QMD | 内置引擎 + 硅基流动 |
|---|---|---|
| Embedding | 本地 EmbeddingGemma 300M | 远端 bge-m3 API |
| Query Expansion | 本地 1.7B 模型 | 无（模型自己提炼关键词） |
| Reranking | 本地 Qwen3-Reranker 600M | 无 |
| 搜索延迟 | 1-2 分钟（2核CPU） | 毫秒级 |
| 中文效果 | ⚠️ EmbeddingGemma 偏英文 | ✅ bge-m3 多语言 |
| 费用 | 免费（但吃 CPU） | 免费（硅基流动免费额度） |
| 硬件要求 | 高（需 GPU 或强 CPU） | 无（计算在远端） |

少了 query expansion 和 reranking 两个环节，搜索质量理论上不如完整的 QMD 流程。但对于我 20-30 个记忆文件的规模，混合搜索已经绰绰有余——关键词搜索覆盖精确匹配，向量搜索兜底语义匹配，两者互补。
