...
categories: ["ai"]
description: "SGD 只是大模型训练中的一个环节。本文系统梳理从原始数据到千亿参数模型的完整训练 pipeline，涵盖数据工程、Tokenization、模型架构、预训练优化技术、以及 RLHF 等 post-training 关键步骤。"
---

很多人在学习机器学习时接触的第一个优化算法就是**随机梯度下降（SGD）**，它确实是深度学习的基石。但当你真正了解大语言模型（LLM）的训练过程，会发现 SGD 只是整个庞大系统中的一个小齿轮。

本文的目标是：**把大模型训练的完整技术栈梳理清楚**，让你对"模型是如何炼成的"有一个系统性认知。
...
互联网上大量重复内容。如果不处理，模型会反复学习同一份数据，浪费算力且容易过拟合。常用SimHash或MinHash在文档级别做近似去重。
...
SGD 在大模型训练中几乎不用，取而代之的是：

- **AdamW**：带权重衰减的 Adam（实际训练 LLM 主流）
- **Adafactor**：比 AdamW 省显存，适合极长上下文
- **LAMB**：适合大 Batch Size 训练（如 2048+）

AdamW 的核心思想：**每个参数有自己的自适应学习率**，而不是全局统一——这让它比 SGD 更适合训练 Transformer。
...
回到最初的问题：**SGD 在大模型训练中扮演什么角色？**

答案是：它是**预训练优化器的一个变种基础**。现代大模型训练用的是 AdamW 而不是原生 SGD，但 SGD 的核心思想——**用梯度信息迭代更新参数**——依然是所有优化器的底层逻辑。

大模型训练是一个涉及数据工程、分布式系统、GPU 架构、强化学习等多个领域的综合工程。理解完整技术栈，比单独学懂 SGD 有意义得多。
